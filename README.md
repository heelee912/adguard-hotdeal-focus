...
세 번째 ruleset은 `refs/tags/gate-v2.0.2`의 생성·수정·삭제를 막고 integration `15368`만 항상 우회시킵니다. 자동화 개인키는 저장소 secret이 아니라 `hdf-main-automation` 환경 secret에만 있으며, live Chromium 증명 job과 secret-bearing push job은 서로 다른 fresh runner입니다. push runner는 bundle의 단일 부모·허용 경로·불변 filter·manifest hash를 독립 재검증하고 secret 노출 직전 remote head를 다시 확인합니다. local gate publish도 직접 쓰지 않고 `hdf-release-publisher`의 보호된 workflow를 dispatch합니다.
...
Apply is authorized by a clean full default-head SHA, byte-identical copies of all three workflows, and the exact GitHub Actions `verify` job/check from integration `15368`. Three reserved rulesets protect PR/CI, immutable fast-forward history, and creation/update/deletion of `refs/tags/gate-v2.0.2` (only integration `15368` bypasses the tag rule). The private key exists only in the default-branch-restricted `hdf-main-automation` environment. Browser proof and secret-bearing push run on fresh separate runners; the latter revalidates the sealed bundle and leases the remote head before secret exposure. Local gate publication dispatches the protected `hdf-release-publisher` workflow instead of writing directly.
...
`--apply` は clean な default-head の full SHA、3 workflow の同一 byte、integration `15368` の正確な `verify` job/check を先に証明します。3つ目の ruleset は `refs/tags/gate-v2.0.2` の作成・更新・削除を禁止し、integration `15368` だけを bypass します。秘密鍵は default branch 限定の `hdf-main-automation` environment secret のみに置き、browser proof と push は別の fresh runner で実行します。push は bundle と remote head を secret 公開前に再検証し、local gate publish は保護された `hdf-release-publisher` workflow を dispatch します。
...
`--apply` 会先证明干净的默认分支完整 SHA、三个 workflow 的字节一致性，以及 integration `15368` 的精确 `verify` job/check。第三个 ruleset 禁止创建、更新或删除 `refs/tags/gate-v2.0.2`，仅 integration `15368` 可 bypass。私钥只存在于限定默认分支的 `hdf-main-automation` environment secret；浏览器证明与含 secret 的 push 使用不同的新 runner。push 会在暴露 secret 前重新验证 bundle 与远端 head，本地 gate publish 只 dispatch 受保护的 `hdf-release-publisher` workflow。
